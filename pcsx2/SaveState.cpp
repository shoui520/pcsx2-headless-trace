...
//	WriteCP0Status(cpuRegs.CP0.n.Status.val);
	...
	// A portable replay binds the complete scheduler/counter state and the
	// deterministic machine configuration. Recalculating VSync here rewrites
	// nextDeltaCounter/nextStartCounter after a successful restore, so only the
	// compatibility-oriented native .p2s route refreshes it from host settings.
	if (refresh_vsync)
