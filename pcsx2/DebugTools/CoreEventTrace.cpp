...
namespace Pcsx2Trace
{
	namespace
	{
		static constexpr std::array<char, 8> TRACE_MAGIC = {'P', 'C', 'S', 'X', '2', 'E', 'V', 'T'};
...
#pragma pack(pop)
		...
		static_assert(sizeof(CoreEventTraceFileHeader) == 48, "Core event trace header size must stay fixed.");
		static_assert(sizeof(CoreEventTraceRecord) == 88, "Core event trace record size must stay fixed.");
		...
		u32 s_entry_pc = 0;
		std::string s_error;
		CoreEventSchedulerCallback s_scheduler_callback = nullptr;
	...
	bool StartCoreEventTrace(const CoreEventTraceConfig& config, Error* error)
	{
		...
		if (!s_trace_file)
		{
			Error::SetStringFmt(error, "Failed to open core event trace output '{}'.", config.output_path);
			return false;
		}
		...
		if (!WriteHeader())
		{
			Error::SetStringFmt(error, "Failed to write core event trace header to '{}'.", config.output_path);
			StopCoreEventTrace();
			return false;
		}
	...
	{
		s_scheduler_callback = callback;
	}

	void RunCoreEventSchedulerCallback()
	{
		if (s_scheduler_callback)
			s_scheduler_callback();
	}
	...
	bool RecordCoreEvent(CoreEventKind kind, CoreEventPhase phase, CoreEventDomain domain,
		CoreEventId event_id, u64 target_cycle, u32 field0, u32 field1, u32 field2,
		u32 field3, u32 field4, u32 field5, u32 field6, u32 field7, u32 field8,
		u32 field9, u32 field10)
	{
