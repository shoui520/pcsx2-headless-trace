...
namespace Pcsx2Trace
{
	...
	bool RecordMemPreEeInstruction(u32 pc)
	{
		...
		const bool sample_this_instruction =
			s_config.sample_on_ee_trace_record ?
				DidEeTraceRecordLastInstruction() :
				((current % SampleInterval()) == 0);
		if (!sample_this_instruction)
		{
			if (s_config.max_instruction_records != 0 &&
				s_instruction_records_seen >= s_config.max_instruction_records)
			{
		...
		if (!WriteSample())
			return true;
		if (s_config.sample_on_ee_trace_record && ShouldEeTraceStopAfterMemTrace())
			return true;
		if (s_config.max_instruction_records != 0 &&
			s_instruction_records_seen >= s_config.max_instruction_records)
		{
